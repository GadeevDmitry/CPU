...
int main(int argc, const char *argv[])
{
    if (argc == 2)
    {
        ...
        if (stat("machine.cpu", &exe_stat) == -1 || exe_stat.st_mtime < src_stat.st_mtime)
        {
            char cmd[101] = "";
            sprintf(cmd, "./Asm %s machine.cpu", argv[1]);
    ...
    {
        char cmd[201] = "";
        sprintf(cmd, "./Asm %s %s", argv[1], argv[3]);
